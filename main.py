    ...
    try:
        if update.effective_chat.id not in chat_ids:
            ...
            if update.message.chat.id in (admin, admin_2, admin_3):
                ...
                update.message.reply_text('Нажмите кнопки снизу',
                                          reply_markup=ReplyKeyboardMarkup([['Продать', 'Наличие'],
                                                                            ['Статистика за день',
                                                                             'Статистика за месяц'],
                                                                            [
        ...
        if update.message.chat.id in (admin, admin_2, admin_3):
                ...
                if update.message.text == 'Добавить линейку':
                    ...
                    update.message.reply_text('Выберите линейку',
                                              reply_markup=ReplyKeyboardMarkup(reply_keyboard,
                                                                               resize_keyboard=True,
                                                                               one_time_keyboard=True))
                elif update.message.text == 'Добавить объявление':
                    if update.effective_chat.id not in chat_ids:
                        context.user_data['locality'][len(context.user_data['locality']) + 1] = 'Добавить объявление'
                        update.message.reply_text(text_start_2, reply_markup=ReplyKeyboardMarkup([['Назад']],
                                                                                                 resize_keyboard=True,
                                                                                                 one_time_keyboard=True))
                else:
            ...
            # обработка xlsx нового закупа
                ...
                if update.message.caption == 'Назад' or update.message.text == 'Назад':
                    ...
                    if update.message.photo != [] and update.message.caption != None:
                        context.user_data['photo'] = update.message.photo[-1].get_file().file_id
                        ...
                        context.user_data['locality'][len(context.user_data['locality']) + 1] = 'Добавить объявление 2'
                        reply_keyboard = [[elem] for elem in cities.keys()]
                        reply_keyboard.append(['Отмена'])
                        update.message.reply_text('Выберите город публикации\n'
                                                  'Правила доступны по нажатию /start',
                                                  reply_markup=ReplyKeyboardMarkup(reply_keyboard,
                                                                                   resize_keyboard=True,
                                                                                   one_time_keyboard=True))
                        ...
                        update.message.reply_text('Вы не прикрепили текст')
                        return start_menu_handler(update, context)
            elif context.user_data['locality'][len(context.user_data['locality'])] == 'Добавить объявление 2':
                if update.message.text == 'Отмена':
                    return start_menu_handler(update, context)
                if update.message.text in cities.keys():
                    reply_keyboard = InlineKeyboardMarkup(
                        [[InlineKeyboardButton('Принять', callback_data='Принять'),
                          InlineKeyboardButton('Отклонить', callback_data='Отклонить')]])
                    if context.user_data['photo'] != '':
                        context.bot.send_photo(chat_id=cities[update.message.text], photo=context.user_data['photo'],
                                               caption=f"{context.user_data['text']}\n\n"
                                                       f"Автор: {'@' + context.user_data['username'] if context.user_data['username'] is not None else ''} " +
                                                       "[" +
                                                       context.user_data['first_name'] +
                                                       "](tg://user?id=" + str(context.user_data['id']) + ") " +
                                                       str(context.user_data['id']),
                                               reply_markup=reply_keyboard, parse_mode="Markdown")
                        context.bot.send_message(chat_id=update.effective_chat.id, text=text_success)
                    else:
                        context.bot.send_message(chat_id=cities[update.message.text],
                                                 text=f"{context.user_data['text']}\n\n"
                                                      f"Автор: {'@' + context.user_data['username'] if context.user_data['username'] is not None else ''} " +
                                                      "[" +
                                                      context.user_data['first_name'] +
                                                      "](tg://user?id=" + str(context.user_data['id']) + ") " +
        ...
        # Меню доставщика
            ...
            if context.user_data['locality'][len(context.user_data['locality'])] == 'Старт':
                if update.message.text == 'Наличие':
                    ...
                    reply_keyboard = [[elem + f' {delyverymen_id[elem]}'] for elem in deliverymen.keys() if elem not in ['Склад']]
                    reply_keyboard.append(['Отмена'])
                    update.message.reply_text('Выберите доставщика',
                                              reply_markup=ReplyKeyboardMarkup(reply_keyboard, resize_keyboard=True,
                                                                               one_time_keyboard=True))
...
def forward_query_handler(update, context):
...
def main():
    ...
    dp.add_handler(CallbackQueryHandler(forward_query_handler, run_async=True))
